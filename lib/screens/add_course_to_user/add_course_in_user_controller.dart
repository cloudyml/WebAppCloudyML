...
class AddCourseInUserController extends GetxController {
  ...
  List<StudentCourseData> studentCoursesByName = [];
    ...
    try {
      ...
      await FirebaseFirestore.instance
        ...
        if (value.docs.isEmpty) {
          ...
          BotToast.showText(
              text:
                  'More Than One Account Found, Please Try Using Mobile Number');
        } else if (value.docs.length == 1) {
    ...
    try {
      ...
      await FirebaseFirestore.instance
        ...
        if (value.docs.isEmpty) {
          ...
          BotToast.showText(
              text: 'More Than One Account Found, Please Delete An Account');
        } else if (value.docs.length == 1) {
          studentData = value.docs[0].data();
  ...
  updateCourseRemove(int index) {
    ...
    studentCoursesByName.removeAt(index);
  ...
  removeCourseFromUserAcc() async {
    try {
      ...
      await FirebaseFirestore.instance
          .collection('Users')
          .doc(studentId)
          .update({'paidCourseNames': studentCoursesById}).whenComplete(() {
 ...
 Future<String> getCourseName({required String courseId})async{
   String name = '';
    try {
     
      await FirebaseFirestore.instance.collection('courses').where('id', isEqualTo: courseId).get().then((value) {
          name = value.docs[0].get('name');
      });
       update(['updateCourseRemove']);
      return name;
    } catch (e) {
             print('Error in get course name $e');
 update(['updateCourseRemove']);
      return name;
    }
  }
  ...
  checkTrialCourseOrNot({required String courseId,required String studentId})async{
    try {
     // await FirebaseFirestore.instance.collection('Users')
    } catch (e) {
         print('Error in check trial $e');
    }
  }
