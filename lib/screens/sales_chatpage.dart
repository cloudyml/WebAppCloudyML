...
//  const SalesChatPage({Key key}) : super(key: key);
    ...
    setState(() {
      if (searchText != "") {
        _collectionStream = FirebaseFirestore.instance
            .collection(
                role == "customer_support" ? 'sales_chats' : "sales_chat")
...
//     _firestore.collection("sales_chat").doc(widget.groupId).update({'time': time});
//     _firestore
//         .collection("sales_chat")
//         .doc(widget.groupId)
...
//  final fb.UploadTaskSnapshot uploadTaskSnapshot = await storageRef.put(filePath).future;
  ...
  Future<void> createusercollection() async {
    ...
    // Query documents where 'xyz' field equals 123
    QuerySnapshot querySnapshot = await groups
        .where('student_id', isEqualTo: FirebaseAuth.instance.currentUser!.uid)
        .get();
    final snapshot = await _firestore
        .collection("Users")
    ...
    if (querySnapshot.docs.isEmpty && role == "student") {
      setState(() async {
        String studentName = namecurrent;
        String documentId = FirebaseAuth.instance.currentUser!.uid;
        CollectionReference students =
            FirebaseFirestore.instance.collection('students');
        await students.doc(documentId).set({
