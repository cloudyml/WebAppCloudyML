...
class CXChatPage extends StatefulWidget {
//  const CXChatPage({Key key}) : super(key: key);

  @override
  State<CXChatPage> createState() => _CXChatPageState();
}
...
class _CXChatPageState extends State<CXChatPage> {
    ...
    setState(() {
      if (searchText != "") {
        _collectionStream = FirebaseFirestore.instance
            .collection(role=="sales_team"?'sales_chats':"cx_chat")
...
//     _firestore.collection("cx_chat").doc(widget.groupId).update({'time': time});
//     _firestore
//         .collection("cx_chat")
//         .doc(widget.groupId)
...
//   }
  void updatetime(DateTime time, String message) {
    _firestore.collection("cx_chat").doc(idcurr).update({'time': time});
    _firestore
        .collection("cx_chat")
        .doc(idcurr)
        .update({'lastmessage': message});
...
//  final fb.UploadTaskSnapshot uploadTaskSnapshot = await storageRef.put(filePath).future;
      try {
        ...
        _firestore.collection("cx_chat").doc(idcurr).collection("chats").add({
      ...
      await FirebaseFirestore.instance
        ...
        for (var i in value.docs) {
          CollectionReference groupsCollection =
              FirebaseFirestore.instance.collection("cx_chat");
